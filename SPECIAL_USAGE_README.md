...
- [03_opencv_giris/webcam_paint.py](03_opencv_giris/webcam_paint.py)

  - `cv2.setMouseCallback`: Pencere üzerindeki fare tıklama ve sürükleme olaylarını dinleme.
  - `cv2.flip(..., 1)`: Webcam görüntüsünü ayna modunda gösterme (sağ-sol ters).
  - Maskeleme Yöntemi: Canvas ve video görüntüsünü `cv2.bitwise_and` ve `cv2.add` ile birleştirme.
  - Renk Paleti: Klavye tuşları (R, G, B) ile dinamik renk değişimi.

- [04_gaussian_blur_opencv/tilt_shift_effect.py](04_gaussian_blur_opencv/tilt_shift_effect.py)

  - `np.zeros(..., dtype=np.float32)`: Gradyan maskesi için float32 matris hazırlığı.
  - `mask_3ch = cv2.merge(...)`: Tek kanallı maskeyi 3 kanala çoğaltma.
  - Lineer Gradyan: `mask[y, :] = y / y1` ile kademeli geçiş oluşturma.
  - HSV Doygunluk Artırma: `cv2.cvtColor(BGR2HSV)` ve S kanalını çarpma (`* 1.4`).

- [05_gaussian_blur_manual/kernel_playground.py](05_gaussian_blur_manual/kernel_playground.py)

  - Özel Kerneller: Sharpen, Emboss, Motion Blur için manuel `np.array` tanımları.
  - `cv2.filter2D(..., -1, kernel)`: Tanımlanan çekirdeği görüntüye uygulama (-1: girdi derinliğini koru).
  - `np.eye(9) / 9.0`: Hareket bulanıklığı için diyagonal birim matris oluşturma.
  - Grid Gösterimi: `np.hstack` ve `np.vstack` ile sonuçları matris gibi dizme.

- [06_traditional_image_processing/shape_detector.py](06_traditional_image_processing/shape_detector.py)

  - `cv2.threshold(..., cv2.THRESH_BINARY_INV)`: Beyaz arka planı siyaha çevirerek nesne bulma.
  - `cv2.findContours(..., cv2.RETR_EXTERNAL)`: Sadece en dıştaki konturları alma (iç delikleri yoksayma).
  - `cv2.approxPolyDP`: Konturu çokgene yaklaştırıp köşe sayısını (vertices) bulma.
  - `cv2.moments`: Şeklin ağırlık merkezini (centroid) hesaplayıp yazı yazma.
  - Geometrik Sınıflandırma: Köşe sayısına (3=Üçgen, 4=Kare/Dikdörtgen) göre mantıksal karar.

- [07_keypoints_features/panorama_maker.py](07_keypoints_features/panorama_maker.py)

  - `cv2.Stitcher_create()`: OpenCV'nin hazır panorama sınıfını başlatma.
  - `stitcher.stitch(imgs)`: Özellik eşleştirme, homografi ve warping işlemlerini tek adımda yapma.
  - Hata Kodları: `cv2.Stitcher_OK` dışındaki durumları (yetersiz örtüşme vb.) kontrol etme.
  - `cv2.resize(..., fx=scale, fy=scale)`: Büyük panoramayı ekrana sığdırmak için oran koruyarak küçültme.

- [08_cnn_intro/data_augmentation_demo.py](08_cnn_intro/data_augmentation_demo.py)

  - `torchvision.transforms.Compose`: Birden fazla dönüşümü zincirleme.
  - `transforms.RandomRotation/HorizontalFlip`: Veri çeşitliliği için rastgele dönüşümler.
  - `transforms.ColorJitter`: Parlaklık, kontrast ve doygunlukla rastgele oynama.
  - `Image.open`: Torchvision uyumluluğu için görüntüyü PIL formatında yükleme.

- [09_numpy_matplotlib/color_distribution_3d.py](09_numpy_matplotlib/color_distribution_3d.py)

  - `plt.figure().add_subplot(..., projection='3d')`: Matplotlib ile 3 boyutlu eksen oluşturma.
  - `img.reshape(-1, 3)`: (H, W, 3) boyutundaki görüntüyü (N, 3) piksel listesine düzleştirme.
  - `np.random.choice`: Performans için piksellerden rastgele örneklem alma.
  - `ax.scatter(r, g, b, c=colors)`: Her noktayı kendi RGB renginde 3D uzaya çizme.

- [10_detection_segmentation/face_eye_detector.py](10_detection_segmentation/face_eye_detector.py)

  - `cv2.CascadeClassifier`: XML tabanlı Haar Cascade modelini yükleme.
  - `cv2.data.haarcascades`: OpenCV ile gelen hazır modellerin yolunu bulma.
  - `detectMultiScale`: Farklı boyutlardaki yüzleri tarama.
  - ROI (Region of Interest): Göz aramasını sadece bulunan yüz bölgesinde (`gray[y:y+h, x:x+w]`) yapma performans optimizasyonu.

- [app_launcher.py](app_launcher.py)
  - `tkinter.ttk`: Modern görünümlü (işletim sistemi temalı) arayüz elemanları.
  - `subprocess.Popen`: Python betiklerini ana arayüzü dondurmadan ayrı işlem olarak başlatma.
  - `filedialog.askopenfilename`: İşletim sisteminin dosya seçme penceresini kullanma.
  - Dinamik Liste: Proje listesini tuple listesi olarak tutup döngüyle butonları oluşturma.
